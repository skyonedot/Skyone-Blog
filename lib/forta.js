...
export function getFortaScoreFromLocally() {
  ...
  for (let i of data) {
    ...
    if (i.split(',')[0] in addressScore_) {
      if (addressScore_[i.split(',')[0]].findIndex(f => f.key === i.split(',')[1]) === -1) { 
        addressScore_[i.split(',')[0]].push({key: i.split(',')[1], value: i.split(',')[2]});
      }
    } else {
      addressScore_[i.split(',')[0]] = [{key: i.split(',')[1], value: i.split(',')[2]}];
    }
